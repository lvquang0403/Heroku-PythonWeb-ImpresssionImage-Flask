...
def huffman(file):
    ...
    print("Your original file size was", uncompressed_file_size,"bits. The compressed1 size is:",compressed_file_size)
    string=string+"Your original file size was " + str(uncompressed_file_size) +" bits. The compressed1 size is: "+str(compressed_file_size) +"<br>"
    print("This is a saving of ",uncompressed_file_size-compressed_file_size,"bits")
    string=string + "This is a saving of " +str(uncompressed_file_size -compressed_file_size) +"bits"
    if not os.path.isdir("./compressed"):
        os.mkdir("./compressed")
    output = open("./compressed/compressed1.txt","w+")
    print("Compressed file generated as compressed1.txt")
    output = open("./compressed/compressed1.txt","w+")
    print("Decoding.......")
    #store shape image
    bitstring = bitstring + "["+str(shape[0]) + "-" +str(shape[1])
    output.write(bitstring)
    #decode
    #get size of image
    # start = bitstring.find("[")+1
    # width = bitstring[start:bitstring.find('-')]
    # height = bitstring[bitstring.find('-')+1:]
    ...
    data.save('./compressed/uncompressed1.png')
    if a.all() == res.all():
        print("Success")
    return string,"compressed1.txt"
    # if h == 2:
    #     temp = re.findall(r'\d+', uncompressed_string)
    #     res = list(map(int, temp))
    #     print(res)
    ...
    # Ref: https://github.com/sudhamshu091/Huffman-Encoding-Decoding-For-Image-Compression